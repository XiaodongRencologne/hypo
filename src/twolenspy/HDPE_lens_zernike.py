...
from Kirchhoffpy import zernike_torch
...
class HDEP_2lens():
    def __init__(self, 
                 ...
                 outputfolder = '',
                 z_order = [0,0],
                 z_coeff_l1 = [None,None],
                 z_coeff_l2 = [None,None]):
        self.freq = freq
        ...
        self.L1 = lenspy.simple_Lens(HDPE,
                                ...
                                z_error_f1 = z_coeff_l1[0],
                                z_error_f2 = z_coeff_l1[1],
                                z_order1=z_order[0],
                                z_order2=z_order[0])
        self.L2 = lenspy.simple_Lens(HDPE,
                                ...
                                groupname = groupname,
                                outputfolder = outputfolder,
                                z_error_f1 = z_coeff_l2[0],
                                z_error_f2 = z_coeff_l2[1],
                                z_order1=z_order[1],
                                z_order2=z_order[1])
    ...
    def read_beam(self,field_name = None,x0 = 0):
        ...
        NN = np.where((np.abs(E_co)**2 + np.abs(E_cx)**2 )/peak > 10**(-30/10))[0]
        #print(np.log10(np.abs(E_co).max()**2)*10,np.log10(np.abs(E_cx).max()**2)*10)
